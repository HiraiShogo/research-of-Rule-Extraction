...
from convolutional_network2 import SimpleConvNet2# @UnresolvedImport
...
if __name__ == '__main__':
    network1 = SimpleConvNet(input_dim=(1,28,28),
                        ...
                        conv2_param = {'filter2_num': 10, 'filter2_size': 20, 'pad2': 0, 'stride2': 1},
    ...
    for file in files:
        ...
        number = int(str(file).replace(path1,'').replace('\\',''))
    ...
    for f1 in files:
        ...
        for f2 in mid:
            ...
            os.makedirs("logic\\"+str(todaydetail)+ "\\middle\\" + str(number) + "\\" + str(num2))
            fw = open(('logic\\'+str(todaydetail) + "\\middle\\" + str(number) + "\\" + str(num2) + "\\result.txt"), "w")
            fw.write("\'Not\' enabled: "+str(Flag)+"\n\n")
            ...
            if len(l) != 0:
                for i in l2:
                    ...
                    for u in range(0, Units):
                        ...
                        for a in network_output2.reshape((len(network_output2[0]))):
                            if(a>=0.5):
                                no2.append(1)
                            else:
                                no2.append(0)
                                out = []
                        print("no2 = "+str(no2))
                        if(mid_layer[number-1].getNum() != 0):
                            input = no2
                            for p in range(len(mid_layer)):
                                out = mid_layer[len(mid_layer)-p-1].output(input)
                                input = out
                        out.append(test_f.culc(no2))
                        print("out = "+str(out))
                        network_output = network_output.reshape((len(network_output[0])))
                        network_output = (network_output*2).astype(np.int)
                        print("network_output = "+str(network_output))
                        for j in range(0,len(out)):
                            network_output[j] = out[j]
                            result = bcol.output(network_output)
                        print("out_input = "+str(network_output))
                        print("result = "+str(result)+" →"+str(t_test[u])+"\n")
                        ok = True
                        for o in range(0,len(result)):
                            if((result[o]==1 and o != t_test[u]) or (result[o]==0 and o == t_test[u])):
                                ok = False
                                break
                        if ok:
                            sum += 1
                    ...
                    fw.write('acc : '+str(acc)+"\n")
                    fw.write("===========================================\n\n")
            mid_layer[number-1].addItem(b,Flag)
